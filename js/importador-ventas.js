...
// Los códigos de 1–2 dígitos del reporte son RUBROS/SUBCATEGORÍAS, no productos:
// ningún PLU del catálogo tiene menos de 3 dígitos. Todo código < 100 es encabezado.
const PLU_MIN = 100;

// ── Parser específico del formato del software ────────────────
// El reporte es JERÁRQUICO, en tres niveles:
//   • RUBRO         = código corto (<100) + nombre + CANTIDAD (es el TOTAL del rubro)
//   • SUBCATEGORÍA  = código corto (<100) + nombre, SIN cantidad
//   • PRODUCTO      = PLU largo (>=100) + nombre + CANTIDAD [+ IMPORTE + Tamanio]
//
// Dos trampas que producían valores mal cargados:
//   1) Las filas de RUBRO traen el total de la categoría (ej. VARIOS 662). El
//      parser viejo las tomaba como productos (solo pedía código>0 y cantidad>0).
//   2) Un mismo producto se RE-LISTA bajo una segunda subcategoría (ej. CREMA
//      CATALANA aparece bajo POSTRES y otra vez bajo "DELICIAS CACERAS"): son las
//      MISMAS ventas mostradas en dos agrupaciones. Sumarlas inflaba el descuento.
//
// Regla (validada contra los totales de rubro del reporte real, 18/18):
//   - Se ignoran las filas de encabezado (código < 100). El RUBRO además reinicia
//     el contexto; la SUBCATEGORÍA solo cambia el grupo actual.
//   - Se cuenta cada (PLU + tamaño) UNA vez por subcategoría. Si el mismo reaparece
//     bajo OTRA subcategoría del rubro, es una re-lista → se ignora.
//   - Dos filas del mismo (PLU + tamaño) en la MISMA subcategoría (dos precios) SÍ
//     suman. Distinto tamaño (Gin Tonic Nacional vs Importado) son líneas distintas.
function parsearFilas(filas) {
  const items = [];
  let subcat = " inicio";       // subcategoría actual (aún ninguna)
  const vistoEn = new Map();          // clave -> subcat donde se contó primero (en el rubro actual)
  ...
  for (const fila of filas) {
    ...
    if (isNaN(codNum) || codNum <= 0) continue;                     // fila vacía / sin código
    if (typeof nombre !== "string" || !nombre.trim()) continue;
    const cant = parseFloat(cantidad);

    // ── Encabezado (rubro o subcategoría): código corto ──
    if (codNum < PLU_MIN) {
      if (!isNaN(cant) && cant > 0) {   // RUBRO (trae total) → arranca contexto nuevo
        vistoEn.clear();
        subcat = " rubro";         // productos "sueltos" bajo el rubro, sin subcategoría
      } else {                          // SUBCATEGORÍA (sin total) → cambia el grupo
        subcat = nombre.trim().toUpperCase();
      }
      continue;                         // los encabezados nunca son ventas
    }

    // ── Producto ──
    if (isNaN(cant) || cant <= 0) continue;
    const clave = String(codNum) + "||" + tamano.toUpperCase();
    if (vistoEn.has(clave) && vistoEn.get(clave) !== subcat) continue;  // re-lista bajo otra subcat
    if (!vistoEn.has(clave)) vistoEn.set(clave, subcat);
    items.push({ plu: String(codNum), nombre: nombre.trim(), cantidad: cant, tamano });
  }

  // Sumar las líneas que quedaron: mismo (PLU + tamaño) en la MISMA subcategoría suma.
  const porClave = {};
